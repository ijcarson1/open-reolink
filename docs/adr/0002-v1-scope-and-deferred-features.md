...
The AI feature is **scoped to the Doorbell only** in v1 — "what's at the door" is the load-bearing use case. Generalising AI to all Cameras (Garden / Jack's room) is deferred; the existing prompt and `VisionAnalyzer` are already byte-generic over JPEG, so this is a UI/prompt scope decision not a code one.
...
- **PTZ on E1 Zoom**: cheap CGI commands (`cmd=PtzCtrl`). Deferred to v1.1 to keep v1 focused.
- **Floodlight + siren on Duo 3 PoE**: also cheap CGI. Deferred to v1.1.
